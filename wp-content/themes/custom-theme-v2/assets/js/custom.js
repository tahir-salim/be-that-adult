$(document).ready(function () {
    ...
    $("[data-targetit]").on("click", function (e) {
        ...
        // Get the target class from the clicked tab
        var target = $(this).data("targetit");

        // Calculate the target position before fadeIn
        var scrollTarget = $("." + target).offset().top + 550;

        // Show the content box associated with the clicked tab with fadeIn
        $("." + target).fadeIn();

        // Assuming you are using slick slider, adjust its position
        $(".slider-tabbing").slick("setPosition", 0);
